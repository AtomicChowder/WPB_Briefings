  ...
  "talking_points": [
    {
      ...
      "why_it_matters": "1 sentence: why this matters to this specific user's role.",
      "bullets": [
        "Bullet 1 — specific fact or data point. Wrap person names: <strong>Name, Title</strong>.",
        "Bullet 2 — another key implication.",
        "Bullet 3 — action or decision this creates."
      ],
      "source_links": [
...
## CRITICAL: Never Write HTML Directly

**NEVER use the Write or Edit tools to create or modify `docs/*/index.html` or any `docs/*/*.html` file.**

`docs/*/index.html` is ALWAYS generated by running:
```
python src/render.py adam
python src/render.py surali
```

Or via the convenience script: `bash bin/briefing-render`

Writing HTML directly produces stale CSS classes, broken links, empty talking points, and a broken HSBC logo — this has caused repeated production failures. `templates/briefing.html` is the single source of truth for all HTML structure, including the HSBC logo SVG.
...
```
templates/briefing.html   — Jinja2 HTML template — ONLY place to change HTML/CSS/logo
src/render.py             — Reads briefing_data.json, renders HTML (no API calls)
src/build_briefing.py     — Builds briefing_data.json from briefing_input.json
src/update_history.py     — Updates context/history.json rolling window
context/history.json      — Rolling 7-day coverage history (committed to repo)
docs/adam/index.html      — GENERATED — never edit directly
docs/adam/briefing_data.json
docs/surali/index.html    — GENERATED — never edit directly
